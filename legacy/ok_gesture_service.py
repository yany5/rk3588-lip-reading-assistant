import cv2
import time
import math
import queue
import threading

from config import OK_HOLD_FRAMES, OK_DETECT_EVERY_N_FRAMES

try:
    import mediapipe as mp
    HAS_MEDIAPIPE = True
except Exception as e:
    print("[WARN] mediapipe 导入失败，OK 手势识别不可用。")
    print("[WARN]", e)
    HAS_MEDIAPIPE = False
    mp = None


def distance(p1, p2):
    return math.sqrt((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2)


class OKGestureService:
    """
    OK 手势接口：
    1. 从 CameraService 获取最新帧
    2. 后台线程检测 OK
    3. 检测到 OK 后往事件队列里放一个事件
    """

    def __init__(self, camera_service, stop_event):
        self.camera = camera_service
        self.stop_event = stop_event

        self.thread = None
        self.ok_events = queue.Queue()

        self.current_ok = False
        self.ok_count = 0
        self.last_ok_time = 0.0

        self.mp_hands = None
        self.mp_draw = None
        self.hands = None

        self.frame_index = 0

    def start(self):
        if HAS_MEDIAPIPE:
            self.mp_hands = mp.solutions.hands
            self.mp_draw = mp.solutions.drawing_utils
            self.hands = self.mp_hands.Hands(
                static_image_mode=False,
                max_num_hands=1,
                model_complexity=0,
                min_detection_confidence=0.6,
                min_tracking_confidence=0.6,
            )
            print("[OK] MediaPipe Hands 已启动。")
        else:
            print("[OK] MediaPipe 不可用，只能用键盘 o 模拟 OK。")

        self.thread = threading.Thread(target=self._loop, daemon=True)
        self.thread.start()
        print("[OK] OKGestureService 已启动。")

    def _loop(self):
        while not self.stop_event.is_set():
            frame = self.camera.get_frame()

            if frame is None:
                time.sleep(0.03)
                continue

            self.frame_index += 1

            if self.frame_index % OK_DETECT_EVERY_N_FRAMES != 0:
                time.sleep(0.005)
                continue

            ok = False

            if HAS_MEDIAPIPE and self.hands is not None:
                ok = self._detect_ok(frame)

            self._push_event_if_needed(ok)

            time.sleep(0.005)

        print("[OK] OKGestureService 线程退出。")

    def _detect_ok(self, frame):
        h, w = frame.shape[:2]
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        result = self.hands.process(rgb)

        if not result.multi_hand_landmarks:
            return False

        hand_landmarks = result.multi_hand_landmarks[0]
        lm = hand_landmarks.landmark

        def pt(i):
            return int(lm[i].x * w), int(lm[i].y * h)

        thumb_tip = pt(4)
        index_tip = pt(8)
        wrist = pt(0)
        middle_mcp = pt(9)

        palm_size = distance(wrist, middle_mcp)
        if palm_size < 1:
            palm_size = 1

        close_ratio = distance(thumb_tip, index_tip) / palm_size

        def finger_extended(tip_id, pip_id):
            return lm[tip_id].y < lm[pip_id].y - 0.02

        middle_ext = finger_extended(12, 10)
        ring_ext = finger_extended(16, 14)
        pinky_ext = finger_extended(20, 18)

        extended_count = sum([middle_ext, ring_ext, pinky_ext])

        return close_ratio < 0.38 and extended_count >= 2

    def _push_event_if_needed(self, ok_detected):
        now = time.time()

        self.current_ok = ok_detected

        if ok_detected:
            self.ok_count += 1
        else:
            self.ok_count = 0

        if self.ok_count >= OK_HOLD_FRAMES:
            if now - self.last_ok_time > 1.2:
                self.last_ok_time = now
                self.ok_events.put(now)
                print("[GESTURE] 检测到 OK 手势")

    def simulate_ok(self):
        self.ok_events.put(time.time())
        print("[KEY] 模拟 OK 事件")

    def clear_events(self):
        while True:
            try:
                self.ok_events.get_nowait()
            except queue.Empty:
                break

    def get_event(self, timeout=0.1):
        try:
            return self.ok_events.get(timeout=timeout)
        except queue.Empty:
            return None

    def wait_release(self, stable_sec=0.45, max_wait=5.0):
        start = time.time()
        release_start = None

        while time.time() - start < max_wait and not self.stop_event.is_set():
            if not self.current_ok:
                if release_start is None:
                    release_start = time.time()

                if time.time() - release_start >= stable_sec:
                    return True
            else:
                release_start = None

            time.sleep(0.05)

        return False

    def stop(self):
        if self.thread is not None:
            self.thread.join(timeout=2)

        if HAS_MEDIAPIPE and self.hands is not None:
            self.hands.close()

        print("[OK] OKGestureService 已停止。")
